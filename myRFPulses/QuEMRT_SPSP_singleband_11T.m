...
filename = 'QuEMRT_SPSP_singleband_11T';

% define field strength
b0_tesla = 4.0; % B0 field strenth in T
...
pass_df_ppm = 1.0; % passband width (ppm)

% choose stopband settings
stop_f_ppm = 13.61; % stopband center freq (ppm)
stop_df_ppm = 9.72; % stopband width (ppm)
...
z_timebandwidth = 1.5;                 % time-bandwidth product of RF pulse
...
mets(2).name = 'passband'; 	mets(2).f = pass_f_hz;      mets(2).df = pass_df_hz; 	mets(2).ang = 90;   mets(2).d = 0.001;
...
clear("b0_tesla", "pass_f_hz", "pass_df_hz", "stop_f_hz", "stop_df_hz", "pass_f_ppm", "pass_df_ppm", "stop_f_ppm", "stop_df_ppm")
...
%% interpolate shapes on different raster
if false % switch to true to execute this block
...
%% plot results for individual metabolites at 11.7 T
b0_plot = 11.7; 
mets_freq_ppm = [12.6 8.4 5.9 0.5 0 -8.6 -9.7];
mets_freq_hz = mets_freq_ppm * 1e-6 * b0_plot * 1e4 * SS_GAMMA;
mets_names = {'Lactate','Pyruvate-hydrate','Alanine','Ethylacetate','Pyruvate', 'Urea', 'Bicarbonate'};
...
clear("mets_freq_ppm", "mets_freq_hz", "mets_names", "idx", "rf_shift", "b0_plot")
